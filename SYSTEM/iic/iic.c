...
volatile unsigned char presshigh,presslow;
...
//		}
		...
		SDA_IN();
		presslow = IIC_Read_Byte_ACK();
		SDA_OUT();
		IIC_Ack();
	
		SDA_IN();
		presshigh=IIC_Read_Byte_NACK();
		SDA_OUT();
		IIC_NAck();
		IIC_Stop();
		return;
...
u8 IIC_Read_Byte_ACK(void)//接收函数，结尾ACK
{
	unsigned char i,receive=0;
	SDA_IN();		
    for(i=0; i<8; i++ )
...
u8 IIC_Read_Byte_NACK(void)//接收函数，结尾NACK
{
	unsigned char i,receive=0;
	SDA_IN();		
    for(i=0; i<8; i++ )
