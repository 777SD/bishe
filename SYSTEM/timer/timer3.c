...
/******************
功能：通用定时器3中断初始化,这里时钟选择为APB1的2倍，而APB1为36M
参数：arr：自动重装值。
			psc：时钟预分频数
返回值：无
******************/
...
/*
 *==============================================================================
功能：TIM3 PWM部分初始化，初始化引脚使其输出PWM信号
参数：arr：自动重装值
			psc：时钟预分频数
			ch: 定时器通道号 范围：1--4
...
返回值：无
应用示例：TIM3_PWM_Init(4999,7199,2,RCC_APB2Periph_GPIOB,GPIOB,GPIO_Pin_5);
 *==============================================================================
 */
void TIM3_PWM_Init(u16 arr,u16 psc,u8 ch,uint32_t RCC_APB2Periph,GPIO_TypeDef* GPIOx,uint16_t GPIO_Pinx)
{
...
//    RCC_APB2PeriphClockCmd(RCC_APB2Periph | RCC_APB2Periph_AFIO, ENABLE);  //使能GPIO外设和AFIO复用功能模块时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph, ENABLE);  //使能GPIO外设和AFIO复用功能模块时钟
