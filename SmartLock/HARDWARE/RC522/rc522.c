...
//    printf("请放置卡片\r\n");
    ...
    oled_ShowCHinese(32,0,55);
    oled_ShowCHinese(48,0,56);
    oled_ShowCHinese(64,0,51);
    oled_ShowCHinese(80,0,52);
    if(ret==1)
    {
        ...
        if(status==MI_OK)
        {
            ...
            if(card.stat==66)
            {
                ...
                oled_ShowCHinese(16,4,51);
                oled_ShowCHinese(32,4,53);
                oled_ShowCHinese(48,4,49);
                oled_ShowCHinese(64,4,50);  
                SystemData.ClearFlag=1;
            }
            //卡已经挂失
            ...
            {
                card.stat=66;
                strncpy((char*)card.phone,"13258171070",sizeof(card.phone)-1);
                card.money=0;
                ...
                if(status == MI_OK)//写卡成功
                {
                    ...
                    oled_ShowCHinese(16,4,54);
                    oled_ShowCHinese(32,4,51);
    ...
    if(!SystemData.ClearFlag) oled_ShowString(16,4,(u8*)"          ",16);
...
返回值：无
******************/
void Remove_CardInformation(void)
{
    u8 ret;
    CARD card;
    ret=Check_Card();//验证卡
    ...
    if(ret==1)
    {
        ...
        {
                card.stat=00;
                strncpy((char*)card.phone,"00000000",sizeof(card.phone)-1);
                card.money=0;
...
void Recharge_CardInformation(uint16_t count)
{
    ...
    if(ret==1)
    {
        ...
        if(status==MI_OK)
        {
            ...
            else if(card.stat==88) printf("此卡已挂失.请先解锁后再充值.\r\n");

            else printf("此卡没有激活.请先激活后再充值.\r\n");
...
void Consumption_CardInformation(uint16_t count)
{
    ...
    if(ret==1)
    {
        ...
        if(status==MI_OK)
        {
            ...
            else if(card.stat==88) printf("此卡已挂失.请先解锁\r\n");

            else printf("此卡未激活.请先激活\r\n");
    ...
    if(ret==1)
    {
        ...
        if(status==MI_OK)
        {
            if(card.stat==66)//检查卡是否激活
            {
                ...
                card.stat=88;
                status = PcdWrite(addr, (u8*)&card);
                if(status == MI_OK)  printf("此卡已成功挂失\r\n");
                else  printf("挂失失败\r\n");
            }
            else if(card.stat==88) printf("此卡已挂失\r\n");
            else printf("此卡没有激活.请先激活.\r\n");
        }
        PcdReset();
    }
}
...
void Unlock_CardInformation(void)
{
    ...
    if(ret==1)
    {
        ...
        if(status==MI_OK)
        {
            if(card.stat==66) printf("此卡已解锁.\r\n");
            else if(card.stat==88)
            {
                card.stat=66;
                status = PcdWrite(addr, (u8*)&card);
                if(status == MI_OK)  printf("此卡已成功解锁\r\n");
                else  printf("解锁失败\r\n");
            }
            else printf("此卡没有激活.请先激活.\r\n");
        }
...
void Verify_CardInformation(void)
{
    u8 ret;
...
//        if(SystemData.WarnFlag) printf("主人有人非法入侵请注意安全");
            PcdReset();
        }
//        else printf("read failed\r\n");
    }
