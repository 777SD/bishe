...
void As608_TCH_Init(void)
{
    GPIO_InitTypeDef  GPIO_InitStructure;
    RCC_APB2PeriphClockCmd(TCH_GPIO_CLK, ENABLE);	 //使能PB,PE端口时钟

    GPIO_InitStructure.GPIO_Pin = TCH_GPIO_PIN;				 //LED0-->PB.5 端口配置
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; 		 //推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
    GPIO_Init(TCH_GPIO_PORT, &GPIO_InitStructure);					 //根据设定参数初始化GPIOB.5
}
...
void Add_FR(void)
{
  static u8 i, ensure,processnum = 0,ID_NUM = 0;
    ...
    //          OLED_ShowCH(0, 4, "                ");
            oled_ShowCHinese(0,2,47);
            oled_ShowCHinese(16,2,48);
...
//    printf("0=< ID <=99\r\n");
    ...
    oled_ShowString(16,0,"S11",16);
    oled_ShowCHinese(48,0,29);
    oled_ShowString(64,0,",",16);
    oled_ShowString(80,0,"S12",16);
    oled_ShowCHinese(112,0,30);
    oled_ShowCHinese(0,2,12);
    oled_ShowString(16,2,"S15",16);
...
static uint8_t SaveIndex=0;//密码按键按下的先后顺序索引
uint8_t SetPassWord[7]={"111111"};
uint8_t CachPassWord[7];//密码缓存数组
void Change_Password(void)
{
    oled_ShowString(0,2,"Set Password",16);
//    while(1)
//    {
//        Connect_Server();
         if(isKey0)//键盘模块第一个按键按下
        {
//            printf("key5 pressed\r\n");
            isKey0=0;
            CachPassWord[SaveIndex]='0';
            SaveIndex++;
        }
        else if(isKey1)
        {
...
//            printf("key8 pressed\r\n");
            isKey9=0;
            CachPassWord[SaveIndex]='9';
            SaveIndex++;
        }        

        if(SaveIndex==7) SaveIndex=0;

//        oled_ShowString(16*(SaveIndex),4,"*",16);
       if(SaveIndex!=0) oled_ShowString(16*(SaveIndex),4,"*",16);
//    }
        if(isKeyD)
        {
            isKeyD=0;
        ...
        else if(isKeyC)
        {
            isKeyC=0;
            CachPassWord[SaveIndex]='\0';
            memcpy(SetPassWord,CachPassWord,7);
...
uint8_t VerifyIndex=0;//密码按键按下的先后顺序索引
uint8_t InputPassWord[7];//输入的密码
uint8_t ErrorCount=0;//输错密码的次数，当输错三次自动退出并触发报警
//验证密码并开门
void Verify_Password(void)
{
    oled_ShowString(0,2,"Input Password",16);
    while(1)
    {
//        Connect_Server();
         if(isKey0)//键盘模块第一个按键按下
...
//            printf("SetPassWord:%s\r\n",SetPassWord);
            if(strcmp((const char*)InputPassWord,(const char*)SetPassWord)!=0)//密码不正确
            {
                memset(InputPassWord,0x00,sizeof(InputPassWord));//先将存放密码的数组清空
                VerifyIndex=0;//数组索引也置0
                ErrorCount++;//输错次数自加
                oled_Clear();
                oled_ShowString(0,0,"Wrong password",16);
                oled_ShowString(0,2,"Input Again!!!",16);
                if(ErrorCount>=3)
                {
                    SystemData.WarnFlag=1;//报警标志位置1
                    FailCount=0;
            ...
            {
                oled_Clear();
                oled_ShowString(0,0,"Right password",16);
                Control_SG90(2000);//开锁
                delay_ms(100);
                SystemData.LockFlag=1;
                memset(InputPassWord,0x00,sizeof(InputPassWord));//先将存放密码的数组清空
                VerifyIndex=0;//数组索引也置0
                ErrorCount=0;//输错次数也置0
                oled_Clear();
                Key.Ensure=0;
                return;//跳出本循环，返回主函数循环
            }
        }
        if(VerifyIndex==7) VerifyIndex=0;
        
//         oled_ShowString(16*(SaveIndex),4,"*",16);
        if(VerifyIndex!=0) oled_ShowString(16*(VerifyIndex),4,"*",16);
        if(isKeyD)
        {
            isKeyD=0;
            VerifyIndex = 0;
